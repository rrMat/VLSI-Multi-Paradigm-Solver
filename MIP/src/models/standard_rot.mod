...
param x_ub := w - min_widths_heights; # upper bound for Coordinates_x
param y_ub := h_ub - min_widths_heights; # upper bound for Coordinates_y
...
# binary variables that say if chip is rotated
...
subject to J_is_right {i in I, j in I: i < j}:
	Coordinates_x[i] + widths[i]*(1 - Rotated[i]) + heights[i]*Rotated[i] - Coordinates_x[j] <= M_horizontal * (1 - Not_overlaps_ind[1,i,j]);

subject to J_is_left {i in I, j in I: i < j}:
	Coordinates_x[j] + widths[j]*(1 - Rotated[j]) + heights[j]*Rotated[j] - Coordinates_x[i] <= M_horizontal * (1 - Not_overlaps_ind[2,i,j]);

subject to J_is_up {i in I, j in I: i < j}:
	Coordinates_y[i] + heights[i]*(1 - Rotated[i]) + widths[i]*Rotated[i] - Coordinates_y[j] <= M_vertical * (1 - Not_overlaps_ind[3,i,j]);
	
subject to J_is_down {i in I, j in I: i < j}:
	Coordinates_y[j] + heights[j]*(1 - Rotated[j]) + widths[j]*Rotated[j] - Coordinates_y[i] <= M_vertical * (1 - Not_overlaps_ind[4,i,j]);
